...
//       team's roll is measured against) taken from state.floorObjectives, which keeps each
//       floor's mission across incursions so that scouted knowledge survives leaving the tower; the objective's briefing is printed under the floor
...
//       since the objective may not be known -- vs the next floor's difficulty). Scouting does
//       NOT happen here: it is its own errand outside the tower (runScoutMission), so the climb
//       prompt offers only yes or no. What a scout claimed is read back from state.floorReports:
//       if the floor turns out to be something other than what they said, a line says so and
//       names them. Increments
