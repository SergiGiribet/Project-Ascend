...
### Retrieve and Rescue as real missions  *(designed 2026-09-02)*

Half the floors in the game are still one roll and three tiers. Hold and Slay work not because they
loop but because each asks a **different question**, and the two remaining types need their own
rather than a third loop with new text.

| | the question |
|---|---|
| **Hold** | they tell you the duration -- *will you last it?* |
| **Slay** | two pools of health -- *will you get there first?* |
| **Retrieve** | the duration is theirs -- *when do they stop?* |
| **Rescue** | the clock is fixed -- *will you arrive in time?* |

**Retrieve: passes, and the party decides how many.** A pass is a round of searching. Each one
either turns the thing up or does not, and every pass that does not raises how much the floor has
noticed them -- which eventually is a wound. Once they have it they can leave with it, or keep
turning the place over: **you have already won the floor, and the question is whether you gamble
what you won.** Extra passes pay extra essence and cost exposure, which is the only variable payout
in the game and the reason the dial means anything.

**The dial is not a number in the menu. It is who you sent.** The Greedy and the Curious want one
more pass; the Cowardly is already at the stairs. The evidence that this is the right reading is a
cell of the trait table written back in July: **Retrieve is the only objective where Cowardly scores
positively (+4)**, and not for perception -- Alert already covers that. A coward scores well at
searching for exactly one reason: they leave before the floor notices. The table has been describing
this mechanic since before we had it.

Rejected on the way: letting the player set a search depth at entry. It turns the entry screen from
a yes/no into two decisions, and it turns greed into a spreadsheet. *"Hollis wants one more pass;
the others are already on the stairs"* is a scene. *"You set the dial to 4"* is not, and GDD 1.1 says
the loss has to belong to somebody.

**Rescue: their clock, your speed.** The captive's countdown is fixed; how fast the party closes on
them is not, and the table says who pushes -- **Loyal +8, Gentle +8** against **Greedy -8,
Spiteful -8**. Failure has two faces no other type has: arrive too late and the captive dies while
you come home whole, or arrive late *and* expensive, having spent people on someone who was already
gone.

The payoff is unique too: **a person**. Freed captives join the roster with their own history and a
hook naming whoever went in for them, feeding the system already in place. Decided 2026-09-02: they
are generated by the CALLER, not inside resolveFloor -- the seam stays clean and the simulation
keeps knowing nothing about a Generator. Later they should be somebody out of the game's own lore
rather than a fresh roll.

So the four types come to pay in three currencies: Hold and Slay pay the floor, Retrieve pays
essence, Rescue pays a person.

**Where this goes at 2e.** The rejected dial comes back in its good form: you give an **order** --
*don't stay past three passes* -- and they may disobey it depending on who they are. That needs
personalities first, because disobedience is only meaningful if the player could have seen it
coming.

**Neither takes input inside `resolveFloor`.** Every decision is either made at entry or made by the
party, so a sortie can still resolve with nobody watching a console -- which is what 2d-6 needs.
...
**2d-4: the economy, decided 2026-08-26 and still unbuilt.** Three summoning tiers where the price
buys the CEILING as well as the odds, and a free cheap summon while fewer than five units live. The
full design and the weight table are in the section above; nothing about it has changed.

It is next because everything else in 2d is now done. Sorties, floors with their own difficulty, a
scout who can be wrong, and all four missions as real mechanics all shipped between 2026-08-26 and
2026-09-03, along with an economy fix (the toll), a loss that costs what the party's state says
rather than a flat life, and a roll that no longer makes the game more deterministic the deeper it
goes.

After it: **save (2d-5), then time (2d-6), then the curve's slope (2d-7)** -- in that order, and
the order matters. Three separate measurements have now pointed at time as the missing dimension:
the informed bot refuses roughly half its attempts for free and takes 0.1 deaths; knowledge buys
survival but not height; training on low floors is unbounded. Until refusing a sortie and grinding
one both cost something, every balance number is provisional.

Deliberately *not* next, and this needed saying out loud on 2026-09-03: **more balance tuning.**
The bench (`bench.py`, described in the [backlog](backlog.md)) makes mission balance cheap to
measure, which made it tempting to keep going -- but tuning has no natural stopping point, and
every number set before sorties cost time gets set again afterwards. Two constraints were fixed and
the rest was left alone deliberately: every mission must be beatable, and the harder ones must be
rarer. Their being unequal is content, not error.
