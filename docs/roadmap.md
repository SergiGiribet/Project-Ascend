...
**Retrieve, as passes the party chooses.** Design settled 2026-09-02 (section above): rounds of
searching, exposure rising with each fruitless one, and the real decision arriving *after* they find
the thing -- leave with it, or keep turning the place over for more. Nobody sets a number; the
Greedy and the Curious push for another pass and the Cowardly is already on the stairs, so the dial
is the party you sent.

It comes first of the two because it is where `floorRoll` gets extracted -- Hold, Slay and now
Retrieve all roll the same way, which is the third use the rule of three was waiting for.

Then **Rescue** (fixed clock, party speed from traits, the freed captive generated by the caller),
then the **summoning tiers**, then save, then time, then the curve.

Standing: 2d-4's tiered summoning and the free draw below five units are decided and specified in
the section above but not built. The trainer slot stays at 250-and-doubling until there are new
numbers.

Deliberately *not* next: the difficulty curve slope. The per-floor swing already widened the band;
what the slope is worth cannot be read until sorties cost time -- and by then half the floors will
have stopped being a single roll, which changes what there is to measure.
